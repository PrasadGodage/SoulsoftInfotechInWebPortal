...
{
  $sql= "SELECT sum(`Balance`) AS CustOutStanding FROM customer WHERE `UserId`=".$_SESSION['id']."";
  $result = mysqli_query($con,$sql);
  $rows = mysqli_fetch_assoc($result);

  if (mysqli_num_rows($result) == 1) {$CustOutstanding=$rows['CustOutStanding'];}else{$CustOutstanding=0;}
//----------------------------------------------------------------------------
  $sql= "SELECT sum(`Balance`) AS SuppOutStanding FROM Supplier WHERE `UserId`=".$_SESSION['id']."";
...
//----------------------------------------------------------------------------
  $sql= "SELECT sum(`StkQty`)As TotQty,sum(`PurchaseValue`)As TotPur,sum(`MRPValue`)As TotMRP,sum(`CashValue`)As TotCash,sum(`CreditValue`)As TotCredit,sum(`OutletValue`)As TotOut FROM Stock WHERE `UserId`=".$_SESSION['id']."";
